...
public class NonWeighingPackingService : CartonService, INonWeighingPackingService
{
     ...
     : base(genericRepository, labelService, miscMasterService)
    {
        ...
        TypeAdapterConfig<Carton, CartonCrudDto>
            .NewConfig()
            .Map(dest => dest.CartonDetailsDtos, src => src.CartonDetails.Adapt<List<CartonDetailsDto>>())
            .AfterMapping((src, dest) =>
            {
                dest.Id = src.Id;
                if (dest.CartonDetailsDtos != null && src.CartonDetails != null)
                {
                    for (var index = 0; index < src.CartonDetails.Count && index < dest.CartonDetailsDtos.Count; index++)
