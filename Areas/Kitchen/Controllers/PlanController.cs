...
public class PlanController : BaseImportController<BmImportDto>
{
    ...
    public async Task<ActionResult> View(int? id, string warehouseOrderNo)
    {
        try
        {
            if(null == id && string.IsNullOrEmpty(warehouseOrderNo))
                throw new Exception("Invalid Id or warehouse order.");
            ...
            if (null == plan)
            {
                ...
                if(null == plan)
                    throw new Exception($"Plan with Id ({id}) or Order ({warehouseOrderNo}) not found");
            }
    ...
    [HttpPost]
    public async Task<ActionResult> DeletePlan(string warehouseOrderNo)
    {
        try
        {
            await _planService.DeletePlanAsync(warehouseOrderNo).ConfigureAwait(false);
